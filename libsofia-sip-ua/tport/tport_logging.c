                    ...
                    su_iovec_t const iov[], size_t iovused, char const *what, char **buffer)
{
   ...
   /* Need a socket (udp mode) or a registered callback (callback mode); the
      serialized buffer is built either way (source comes from the tport addr,
      not the capture socket). */
   if (!mr->mr_capt_sock && !mr->mr_capt_callback) {
         su_log("error: capture socket is not open\n");
         return 0;
   }
